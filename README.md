...
- **Criterio del Prototipo:** El script busca de forma automatizada malas prácticas. En este entorno (TypeScript), se penaliza la presencia de logs olvidados (`console.log`, `console.error`), tipos inseguros (`any`) y evasiones del compilador (`@ts-ignore`).
...
- **Justificación de Calidad:** Archivos extremadamente largos violan el "Principio de Responsabilidad Única" (SRP). Un índice alto advierte a los arquitectos de software que el sistema es frágil, difícil de probar (baja testabilidad) y complejo para que los nuevos programadores lo entiendan.
